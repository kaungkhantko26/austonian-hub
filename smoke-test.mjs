...
check(styles.includes('padding:calc(env(safe-area-inset-top) + 12px) clamp(16px,4.975vw,20px) calc(88px + env(safe-area-inset-bottom))'), 'Mobile content does not have a single responsive navigation allowance');
check(styles.includes('left:clamp(10px,5vw,20px)!important;right:clamp(10px,5vw,20px)!important;bottom:calc(8px + env(safe-area-inset-bottom))!important'), 'Mobile bottom navigation does not match the reference floating safe-zone inset');
